    ...
    <v-row justify="center">
      <v-col cols="12" sm="8" md="6">
        ...
        <transition v-if="!registered">
          <v-card class="mx-auto glossy" max-width="500" :class="{'short': isLoginForm, 'tall': !isLoginForm}" style="padding: 5px;">
            ...
            <v-card-actions v-if="!isLoading">
              <v-btn color="primary" @click="isLoginForm ? login() : register()" class="mr-4">{{ isLoginForm ? 'Login' : 'Register' }}</v-btn>
              <v-btn color="secondary" @click="toggleForm">{{ isLoginForm ? 'Go to Register' : 'Back to Login' }}</v-btn>
            </v-card-actions>
            <v-progress-circular indeterminate color="primary" v-if="isLoading" style="margin-left: 10px;"></v-progress-circular>
          </v-card>
        </transition>
      </v-col>
    </v-row>
...
@-webkit-keyframes AnimationName {
    0%{background-position:0% 50%;background: linear-gradient(200deg, #800096, #9300c8, #8a00e2, #4b0082);}
    50%{background-position:100% 50%;background: linear-gradient(250deg, #800096, #9300c8, #8a00e2, #4b0082);}
    100%{background-position:0% 50%;background: linear-gradient(300deg, #800096, #9300c8, #8a00e2, #4b0082);}
}
@-moz-keyframes AnimationName {
    0%{background-position:0% 50%;background: linear-gradient(100deg, #800096, #9300c8, #8a00e2, #4b0082);}
